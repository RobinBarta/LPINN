...
class CustomLoggingCallback(keras.callbacks.Callback):
    def __init__(self, test_data, model, N_epochs, log_dir):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        mae = self.model.calc_MAE(y_true, y_pred)
        cor = self.model.calc_PCC(y_true, y_pred)
...
class DataProcessor:
    def __init__(self, data):
        self.inputs, self.y_true = data
    def normalize_time(self): # Normalizes the first column of the inputs (time) to be between 0 and 1
        t = self.inputs[:,0]
        t_min, t_max = np.min(t), np.max(t)
        self.inputs[:,0] = (t - t_min) / (t_max - t_min)
        print(f'Time array: Nt={len(np.unique(self.inputs[:,0]))}', np.unique(self.inputs[:,0]))
    def shift_time(self): # only subtract t_min
        t = self.inputs[:,0]
        t_min, t_max = np.min(t), np.max(t)
        self.inputs[:,0] = t - t_min
        print(f'Time array: Nt={len(np.unique(self.inputs[:,0]))}', np.unique(self.inputs[:,0]))
    def normalize_Temp(self): # shifts T from inbetween -0.5 and 0.5 to the 0 and 1
        self.y_true[:,3] += 0.5  
    def process(self):
        self.shift_time()
        #self.normalize_Temp()
        return [self.inputs, self.y_true]
        
    
def activation_function(x):
    return 0.5 * tf.sin(2.0*np.pi*x)
...
class PINN(keras.Model):
    ...
    def loss_function(self, inputs, y_true):
        ...
        # --------------------------------
        loss_data = keras.losses.mean_squared_error(tf.concat([u_true, v_true, w_true], axis=-1), 
                                                    tf.concat([u, v, w], axis=-1))
        
        # --------------------------------
        # boundary loss for Temperature
        # --------------------------------
        # take the time from the inputs batch and combine it with random yz-data at the boundaries x=0 or x=1
        N = tf.shape(inputs)[0]
        # prepare time array for new inputs array
        t_reshaped = tf.reshape(t, (N,1))
        # boundaries of cell in vertical direction
        z0, z1 = 0.0, 1.0
        # create z-boundary data (cool/heating plate) and random xy-points
        ...
        # predict T-data on x-boundary positions
        y_pred_bound = self(inputs_bound, training=False)
        #_,_,_,T_bound,_ = tf.unstack(y_pred_bound, axis=1)
        T_bound = y_pred_bound[...,3]
        # generate ground truth for the boundaries (known)
        # min and max Values of Temperature
        T_min, T_max = -0.5, 0.5
        # replace data of z_bound with corresponding T boundary values
        T_true_bound = tf.reshape(tf.where(z_bound == z0, T_max, tf.where(z_bound == z1, T_min, z_bound)), (N,))
        # evaluate Temperature at the bounds
        ...
        total_loss = self.lambda_data * loss_data + self.lambda_T_bound* loss_T_bound + self.lambda_conti * loss_conti + self.lambda_NSE * loss_NSE + self.lambda_EE * loss_EE
    ...
    def calc_PCC(self, y_true, y_pred):
        u_true, v_true, w_true, T_true, p_true = tf.unstack(y_true, axis=1)
        u, v, w, T, p = tf.unstack(y_pred, axis=1)
        PCC = { 'COR_u': self.pearson_correlation(u_true, u),
                'COR_v': self.pearson_correlation(v_true, v),
                'COR_w': self.pearson_correlation(w_true, w),
                'COR_T': self.pearson_correlation(T_true, T),
                'COR_p': self.pearson_correlation(p_true, p)}
        return PCC
